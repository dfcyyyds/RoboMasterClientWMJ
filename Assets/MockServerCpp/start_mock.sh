...
CODEC_ARG="hevc"
GOP_ARG="15"
# Default to RealTest.mp4
VIDEO_ARG="$SCRIPT_DIR/RealTest.mp4"
VIDEO_PROVIDED=0
INTERVAL_ARG="1000"
FORCE_REBUILD=0
START_GUI=0
...
if [[ $VIDEO_PROVIDED -eq 0 ]]; then
  LOCAL_REAL="$MOCK_ROOT/RealTest.mp4"
  ...
  if [[ -f "$LOCAL_REAL" ]]; then
    VIDEO_ARG="$LOCAL_REAL"
    echo "[start_mock] Auto-selected RealTest video: $VIDEO_ARG"
