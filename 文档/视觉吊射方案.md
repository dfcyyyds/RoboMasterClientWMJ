...
flowchart TD
  ...
  Sync --> Header["解析包头 (6B)<br/>frame_type, frame_id, frag_idx, payload_len"]
  Header --> Read["读取载荷+包尾"]
...
static constexpr int      MAX_PAYLOAD      = 249;
...
// ---------- 数据包组装 ----------
struct Packet {
    uint8_t buf[300];  // SYNC(1) + 包头(5) + payload(≤249) + XOR(1) + END(1)
    int     len = 0;
};
...
static Packet build_packet(uint8_t frame_type, uint16_t frame_id,
    ...
    pkt.buf[p++] = payload_len;              // 5: PAYLOAD_LEN
    ...
    void tick_redundancy_slot() {
        if (part2_pending) {
            ...
            Packet pkt = build_packet(FT_I_PART2, frame_id, 0,
                                      part2_payload, (uint8_t)part2_len);
            serial.send(pkt);
            frame_id++;
            part2_pending = false;
        } else {
