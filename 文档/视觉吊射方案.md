...
└──────────────────────────────────────┬──────────┘
                                       │
              裁判系统串口 → 图传模块 (300B × 50Hz)
                                       │
┌──────────────────────────────────────┴──────────┐
│               图传服务器 (192.168.12.1)          │
│                                                  │
│  [裁判系统数据接收] → [封装为 Protobuf]           │
│                           │                      │
│                    [MQTT Broker 发布]             │
│                    Topic: "CustomByteBlock"       │
└──────────────────────────────────────┬──────────┘
                                       │
                   MQTT over TCP (QoS 1, 局域网WiFi)
                                       │
┌──────────────────────────────────────┴──────────┐
│           接收端 (Unity 自定义客户端)              │
│                                                  │
│  [MqttClientService]                             │
│        │                                         │
│  [CustomByteBlockHandler] → 解析 Protobuf        │
│        │                                         │
│  [ProtobufManager] → 触发 OnDataUpdated          │
│        │                                         │
│  [吊射视觉服务] → 解包校验 → 解码引擎             │
│        │                                         │
│  [帧缓冲区合并] → Texture2D 更新                  │
│        │                                         │
│  [RawImage 渲染 + 轨迹叠加 (LineRenderer)]        │
│        │                                         │
│  [操作手 Canvas UI]                               │
└─────────────────────────────────────────────────┘
```
...
| 项目         | 最低要求    | 推荐                         |
| ------------ | ----------- | ---------------------------- |
| 分辨率       | ≥ 192×144   | 640×480 (下采样到192×144)    |
| 帧率         | ≥ 30 FPS    | 60 FPS                       |
| 接口         | USB / MIPI  | USB UVC                      |
...
| **轨迹缓存**      | 60 帧           | 保留最近 2.4 秒弹道          |
| **冗余slot**      | 25 Hz           | 奇数slot用于重传/控制        |
| **显示放大倍率**  | ×4              | 最终显示 768 × 576           |
| **放大算法**      | Point / Scale2x | Point=默认, Scale2x=推荐     |
| **显示轨迹**      | 彩色渐变叠加    | 蓝(旧)→红→黄(新), UI Graphic |
| **包头开销**      | 8 Byte          | 含同步字+校验+结束符         |
| **有效载荷上限**  | 249 Byte        | 保守设计, 留余量             |
| **发送端CPU占用** | < 1%            | 纯整数位操作                 |
| **压缩延迟**      | < 100 μs        | 单帧压缩耗时                 |
...
```
字节偏移:  0      1           2-3        4          5           6..N-2    N-1   N
        +------+------------+----------+----------+------------+---------+-----+----+
        | SYNC | FRAME_TYPE | FRAME_ID | FRAG_IDX | PAYLOAD_LEN| PAYLOAD | XOR | END|
        | 0xA5 | 1 Byte     | 2 Byte   | 1 Byte   | 1 Byte     | 可变    | 1B  |0x5A|
        +------+------------+----------+----------+------------+---------+-----+----+
        |<------------- 包头 6 Byte ------------->|             |<-- 包尾 2B -->|
        
总长度 = 6 (包头) + PAYLOAD_LEN (载荷) + 2 (包尾) ≤ 257 Byte (≤ 300B 限制)
有效载荷最大 = 249 Byte
```
...
| `payload_len` | 5    | 1 B  | uint8  | 有效载荷字节数, 0~249         |
...
实现: 通过 CustomControl 上行消息发送 (Protobuf: CustomControl { bytes data; })
      CustomControl 上行最大 30 字节, 足以容纳此控制帧 (仅8B包头包尾)
...
─────────────────────────────────────────────────────────────
二值RLE       15:1 ~ 40:1     ~10μs     ~8μs       极低
Huffman       12:1 ~ 25:1     ~50μs     ~30μs      中等
LZ4           10:1 ~ 20:1     ~40μs     ~15μs      中等(需库)
LZ77/Deflate  15:1 ~ 30:1     ~200μs    ~100μs     高(需库)
JBIG          20:1 ~ 50:1     ~500μs    ~300μs     高(需库)
...
capture_frame()
      │
      ▼
试算单包压缩 → compressed_size = compress_single(frame)
      │
      ├─ compressed_size ≤ 249B → 发送 I_FRAME_SINGLE (单包)
      │                          占用1个slot, 最优情况
      │
      └─ compressed_size > 249B → 启动渐进式I帧
              │
              ├── Slot N:   发送 I_FRAME_PART1 (缩略帧)
              │
              └── Slot N+1: 发送 I_FRAME_PART2 (差分补丁)
                            若补丁也超限 → 丢弃最不重要的块
                            (按块到画面中心的距离排序, 优先保留中心区域)
```
...
```
最坏情况 (弹丸运动中, 每帧都有变化):
  D帧: 1B(头) + 4块×10B + 5B(弹丸) + 10点×2B = 66B + 8B(包头尾) = 74B
  I帧Part1: ~90B + 8B = 98B
  I帧Part2: ~200B + 8B = 208B
  
  1秒总计: 23×74B(D帧) + 2×74B(轨迹帧) + 98B(I_P1) + 208B(I_P2)
         = 1702 + 148 + 98 + 208
         = 2,156 Byte/s

  带宽利用率: 2,156 / 15,000 = 14.4%

最好情况 (弹丸静止/无发射):
  空D帧: 8B (仅包头包尾)
  
  1秒总计: 24×8B + 98B + 208B = 498 Byte/s
  带宽利用率: 498 / 15,000 = 3.3%
...
```
           MQTT Broker (图传服务器)
                    │
                    │ Topic: "CustomByteBlock"
                    │ Payload: Protobuf 序列化字节
                    ▼
         ┌─────────────────────────┐
         │  MqttClientService      │  M2Mqtt v4.3.0, QoS 1
         │  (收到 MQTT 消息)        │  自动重连机制
         └────────────┬────────────┘
                      │ byte[] payload
                      ▼
         ┌─────────────────────────┐
         │  MessageDispatcher      │  根据 Topic 路由到对应 Handler
         └────────────┬────────────┘
                      │
                      ▼
         ┌─────────────────────────┐
         │  CustomByteBlockHandler │  CustomByteBlock.Parser.ParseFrom(payload)
         │  (解析 Protobuf)        │  提取 ByteString Data
         └────────────┬────────────┘
                      │ CustomByteBlock message
                      ▼
         ┌─────────────────────────┐
         │  ProtobufManager        │  UpdateData<CustomByteBlock>(message)
         │  (更新数据 + 触发事件)   │  触发 OnDataUpdated 委托
         └────────────┬────────────┘
                      │ OnDataUpdated event
                      ▼
         ┌─────────────────────────┐
         │  LobShotVisionService   │  订阅 OnDataUpdated
         │  (吊射视觉服务, 待实现)  │  从 ViewModel 获取 Data 属性
         └────────────┬────────────┘
...
```
         收到 CustomByteBlock.Data (byte[])
                     │
                     ▼
            ┌──────────────────┐
            │  寻找同步字 0xA5  │  从 byte[] 头部开始扫描
            └────────┬─────────┘
                     │
                     ▼
            ┌──────────────────┐
            │  解析包头 (6B)    │  frame_type, frame_id, frag_idx, payload_len
            └────────┬─────────┘
                     │
                     ▼
            ┌──────────────────┐
            ...
            └────────┬─────────┘
                     ...
                     ▼
            ┌──────────────────┐
            │  校验 XOR + 0x5A  │──── 失败 → 丢弃, 记录错误计数
            └────────┬─────────┘
                     │ 成功
                     ▼
            ┌──────────────────┐
            │  检查frame_id连续 │──── 不连续 → 记录丢帧计数
            └────────┬─────────┘
                     │
        ┌────────────┼────────────────┬──────────────┐
        ▼            ▼                ▼              ▼
 I_FRAME_PART1  I_FRAME_PART2    D_FRAME        TRAIL_FRAME
        │            │                │              │
        ▼            ▼                ▼              ▼
 RLE解码到       块补丁叠加到      XOR合并到       更新轨迹
 临时byte[]      帧缓冲byte[]     帧缓冲byte[]    点List<>
 + 上采样2x           │                │              │
 填充帧缓冲           │                │              │
        │            │                │              │
        └────────────┴────────────────┴──────────────┘
...
private TrailPoint[] _trailBuf = new TrailPoint[120];
...
> GPU 纹理上传耗时 < 0.1ms。`FilterMode.Point` 确保放大时保持像素锐利边缘，
...
Canvas (Screen Space - Overlay)
  │
  ├── LobShotPanel (Panel, 全屏或指定区域)
  │     │
  │     ├── BackgroundFrame (RawImage, 768×576)
  │     │     ├── texture = _displayTex (192×144 R8)
  │     │     ├── RectTransform: 768×576 (4倍放大)
  │     │     └── FilterMode.Point → 最近邻放大, 无模糊
  │     │
  │     ├── TrailOverlay (自定义UI组件或LineRenderer)
  │     │     ├── 绘制弹丸轨迹连线
  │     │     ├── 颜色渐变: 蓝(旧) → 红 → 黄(新)
  │     │     └── 坐标从192×144映射到768×576显示空间
  │     │
  │     ├── BallMarker (Image, 弹丸当前位置标记)
  │     │     ├── 跟随最新弹丸坐标
  │     │     └── 高亮圆圈或十字准星样式
  │     │
  │     └── HUD (Text / TextMeshProUGUI)
  │           ├── 帧号 / 丢帧计数
  │           ├── 弹丸坐标 (cx, cy)
  │           ├── 弹丸速度估算 (像素/帧)
  │           └── 连接状态 / 带宽利用率
...
```csharp
// === 方案A: 使用 Unity UI.Graphic 自定义网格 ===
// 适合 Canvas Space, 零额外 Draw Call (合批)

public class TrailRenderer : MaskableGraphic
{
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        {
            float ratio = (float)i / (_points.Count - 1);
            Color color = GetTrailColor(ratio);
            // 在 _points[i] 和 _points[i+1] 之间绘制线段
    ...
    private Color GetTrailColor(float ratio)
    {
        // ratio: 0.0(最老) → 1.0(最新)
        if (ratio < 0.5f)
            return Color.Lerp(Color.blue, Color.red, ratio * 2f);
        else
            return Color.Lerp(Color.red, Color.yellow, (ratio - 0.5f) * 2f);
    }
}

// === 方案B: 使用 LineRenderer (世界空间, 更简单) ===
// LineRenderer 设置: widthMultiplier=2, colorGradient=蓝→红→黄
// 每次轨迹更新时设置 positionCount 和 SetPositions()
// 需要额外注意 Canvas 坐标与世界坐标的转换
```
...
```
┌────────────────┬────────────────┬──────────┬────────────┬────────────────────────┐
│ 算法           │ GPU 耗时       │ 实现难度  │ 二值图效果  │ 备注                   │
│                │ (768×576输出)  │          │            │                        │
├────────────────┼────────────────┼──────────┼────────────┼────────────────────────┤
│ Point          │ 0 ms           │ 一行代码  │ ★★★★☆     │ Unity 原生支持          │
│ (最近邻)       │ (GPU硬件插值)  │          │ 锐利方块    │ FilterMode.Point       │
├────────────────┼────────────────┼──────────┼────────────┼────────────────────────┤
│ Bilinear       │ 0 ms           │ 一行代码  │ ★☆☆☆☆     │ Unity 原生支持          │
│ (双线性)       │ (GPU硬件插值)  │          │ 边缘灰色带  │ 二值图禁用!            │
├────────────────┼────────────────┼──────────┼────────────┼────────────────────────┤
│ Scale2x ×2级联 │ < 0.01 ms      │ Fragment │ ★★★★★     │ 2次级联实现4×放大       │
│ (EPX 边缘增强) │               │ Shader   │ 平滑对角线  │ 消除阶梯感, 保持锐利   │
├────────────────┼────────────────┼──────────┼────────────┼────────────────────────┤
│ HQ4x           │ < 0.01 ms      │ Shader   │ ★★★★★     │ 需要 LUT 纹理          │
│ (高质量4倍)    │               │ + LUT    │ 最佳边缘    │ 最佳视觉质量           │
├────────────────┼────────────────┼──────────┼────────────┼────────────────────────┤
│ FSR / STP      │ N/A            │ N/A      │ N/A        │ 不适用!                │
│ (时域超分)     │               │          │            │ 仅用于3D渲染管线       │
└────────────────┴────────────────┴──────────┴────────────┴────────────────────────┘
```
...
```
┌────────────────────────────────────────────────────────────────────┐
│  Level 0 (默认): FilterMode.Point                                 │
│  ─────────────────────────────────────────                        │
│  实现: _displayTex.filterMode = FilterMode.Point;                 │
│  效果: 每个源像素变成4×4的纯色方块, 边缘呈直角阶梯状              │
│  优点: 零代码量, 零GPU开销, 二值图天然适配                        │
│  缺点: 对角线边缘有明显锯齿 (像素块感)                           │
│  适用: 快速原型 / 对画质要求不高的场景                             │
├────────────────────────────────────────────────────────────────────┤
│  Level 1 (推荐): Scale2x 级联 (自定义 Shader)                     │
│  ─────────────────────────────────────────                        │
│  原理: 对每个像素检查上下左右4邻域, 智能决定2×2子像素颜色         │
│        级联两次: 192×144 → 384×288 → 768×576                     │
│  效果: 对角线边缘自动平滑, 直线边缘保持锐利                      │
│  实现: URP 自定义 Fragment Shader + 中间 RenderTexture             │
│  GPU耗时: < 0.01ms (768×576 仅 442,368 texel, 每个采样4次)       │
│  适用: 正式比赛方案, 最佳 性价比                                  │
├────────────────────────────────────────────────────────────────────┤
│  Level 2 (极致): HQ4x (自定义 Shader + LUT)                       │
│  ─────────────────────────────────────────                        │
│  原理: 检查 3×3 邻域(8像素), 对256种边缘模式各自定义插值规则      │
│  效果: 最佳边缘质量, 曲线更平滑                                   │
│  实现: Fragment Shader + 256×256 LUT 纹理 (预计算)                │
│  GPU耗时: < 0.01ms (纹理查找, 无分支)                             │
│  适用: 对画质有极致追求时                                          │
└────────────────────────────────────────────────────────────────────┘
```
...
```
768 × 576 显示区域 (192×144 放大4倍, FilterMode.Point)
┌──────────────────────────────────────────────────────┐
│  Frame: 1247  Lost: 0  Ball: (98,45) V: 2.3 px/f    │  ← HUD (TextMeshProUGUI)
│                                                      │
│                                                      │
│                    ██                                 │
│                   ██   ← 弹丸 (白色方块, 约12~20显示像素)│
...
若启用 Scale2x: 对角线边缘由阶梯状 ██ 变为平滑斜线 ▓▒
GPU 开销增加 < 0.01ms, 视觉品质显著提升
```
...
数据结构:
  trail_points[60]  — 最多存储60个点 (覆盖2.4秒弹道)
  trail_head         — 环形缓冲区头指针
  trail_count        — 当前有效点数

添加新点 (每收到 ball_detected=1 的D帧):
  trail_points[trail_head] = {ball_cx, ball_cy}
  trail_head = (trail_head + 1) % 60
  if (trail_count < 60) trail_count++

清空轨迹:
  当检测到新的发射序列时 (例如连续10帧无弹丸后再次出现)
  trail_count = 0, trail_head = 0
...
[MQTT 网络传输]
TCP 投递 (局域网 WiFi)      1~5 ms        QoS 1, 含 ACK
                                          典型 < 2 ms
...
可选: Scale2x Shader    < 0.01 ms (GPU)    +162 KB VRAM    中间 RT 384×288
可选: HQ4x Shader       < 0.01 ms (GPU)    +256 KB VRAM   LUT 256×256
...
│  │ 算法       │ 画质评分 │ 实际观感                               │  │
│  ├────────────┼──────────┼────────────────────────────────────────┤  │
│  │ Point      │ ★★★★☆  │ 像素方块感明显, 但弹丸/靶标轮廓清晰  │  │
│  │            │          │ 类似像素风格游戏, 操作手完全可用      │  │
│  ├────────────┼──────────┼────────────────────────────────────────┤  │
│  │ Scale2x    │ ★★★★★  │ 对角线边缘平滑, 弹丸轨迹弧度更自然  │  │
│  │ (推荐)     │          │ 接近 CRT 显示器上的低分辨率观感      │  │
│  ├────────────┼──────────┼────────────────────────────────────────┤  │
│  │ HQ4x       │ ★★★★★+ │ 最佳边缘质量, 曲线近乎平滑           │  │
│  │            │          │ 代价: 需实现 LUT, 开发量略大         │  │
│  └────────────┴──────────┴────────────────────────────────────────┘  │
...
│    轨迹:  4~8 显示像素宽, 弧度方向清晰可辨                          │
│    靶标:  80~160 显示像素宽, 轮廓清晰, 可判断偏移方向和距离          │
...
结论: 
  ...
  但这些开销在 MQTT/WiFi 链路上传输, 不影响裁判系统物理层的 300B×50Hz 预算
```