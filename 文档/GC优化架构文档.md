...
```mermaid
flowchart TB
    AP_NALU["ArrayPool (NALU)"]
    AP_PIX["ArrayPool (Pixels)"]

    subgraph UDP接收层
        UVH["UdpVideoHandler"]
        NALU["nalu byte[]"]
        UVF["UdpVideoFrame"]
        UVH -->|Rent| AP_NALU --> NALU --> UVF
    end

    subgraph 组帧层
        AFA["AnnexBFrameAssembler"]
        FB["FrameBuffer"]
        SD["SliceData\n(存储池化标志)"]
        MS["MemoryStream\n(复用)"]
        ANNEXB["annexB byte[]"]
        ...
        AFA --> FB
        AFA --> SD
        FB -->|组帧完成| MS -->|ToArray| ANNEXB
        SD -->|Return 分片| AP_NALU
    end

    subgraph 解码层
        FDEC["FfmpegPipeDecoder"]
        PIX["pixels byte[]"]
        DF["DecodedFrame"]

        ANNEXB --> FDEC
        FDEC -->|Rent| AP_PIX --> PIX --> DF
    end
    ...
    subgraph 显示层
        ...
        DF --> VSS -->|LoadRawTextureData| TEX
        DF -->|ReturnToPool| AP_PIX
    end
```
