...
> 详细方案文档见：[视觉吊射方案.md](视觉吊射方案.md) v2.3

### 12.1 背景

吊射图传从 v1.x 二值化方案（192×144, 1bit, 自定义 RLE/XOR 编码）升级为 v2.3 H.264 彩色方案（360×540, 2:3 宽高比, YUV420 彩色, x264 编码）。比赛模式默认 100kbps 彩色，带宽拥塞时自动降级为 80kbps 灰度；仿真模式 200kbps 彩色。客户端解码复用主图传已有的解码基础设施。解码后通过 LobShot-SRGAN 轻量超分辨率网络（2× 上采样）重建至 720×1080，在不增加传输带宽的前提下显著提升画面清晰度。
...
- **v3.3**: 吊射图传分辨率升级 + 超分辨率重建 (2026-04-08)
  - 分辨率从 400×400 调整为 360×540（2:3 宽高比），适配竖屏靶标场景
  - 新增 LobShot-SRGAN 2× 超分辨率重建（360×540 → 720×1080），客户端侧实时推理
  - 更新 §12.1/§12.4/§12.5 相关分辨率参数