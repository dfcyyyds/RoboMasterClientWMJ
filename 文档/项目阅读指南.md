# RoboMaster 选手端客户端 —— 项目阅读指南

> **项目规模（截止2026_0422）**：147 个手写 C# 源文件，约 19 900 行代码（不含自动生成的 Protobuf 代码）。
...
**学习建议**：先读自动路径理解框架原理，再读手写路径学习实际应用。
...
sequenceDiagram
    ...
    participant SUC as StartupUpdateChecker<br/>[-3500]
    ...
    Unity->>SUC: RuntimeInitializeOnLoadMethod()
    Note right of SUC: 拉取远端 manifest<br/>与本地 manifest.local.json 比对
    alt 有更新可用
        SUC->>SUC: 弹出提示弹窗<br/>用户选择立即更新
        SUC->>SUC: 下载变更文件<br/>SHA-256 校验 → 暂存
        SUC->>SUC: 派发平台交换脚本
        SUC->>Unity: Application.Quit()
    else 无更新 / 跳过 / 禁用
        SUC->>SUC: Completed = true
    end
...
### 阶段 A：热身 —— 理解数据契约（约 1 小时）

> **目标**：理解客户端与服务器之间传输的是什么数据。

| 序号 | 文件                                          | 行数 | 要点                                                                       |
| ---- | --------------------------------------------- | ---- | -------------------------------------------------------------------------- |
| A1   | `Assets/Proto/RoboMasterClientMessage.proto`  | 313  | Protobuf 协议定义，35 个消息类型。**先通读一遍**，建立对数据结构的整体印象 |
| A2   | `Scripts/Framework/Network/GameEventTypes.cs` | 83   | 比赛事件 ID 枚举，了解 RoboMaster 比赛有哪些事件                           |
| A3   | `StreamingAssets/Config/params.json`          | 16   | 运行时配置项，理解 IP/端口/分辨率等参数含义                                |
| A4   | `Utils/ConfigData.cs`                         | 39   | 配置数据模型，对应 JSON 字段                                               |

**学习收获**：掌握 Protobuf 协议定义语法；理解客户端需要处理的全部数据类型。

---

### 阶段 B：地基 —— 网络通信框架（约 3 小时）

> **目标**：理解消息从网络到达应用层的完整链路。

| 序号 | 文件                                              | 行数 | 难度 | 要点                                                                                                                      |
| ---- | ------------------------------------------------- | ---- | ---- | ------------------------------------------------------------------------------------------------------------------------- |
| B1   | `Framework/Network/IMessageHandler.cs`            | 20   | 入门 | **从这里开始。** 两个接口，定义了消息处理的标准契约                                                                       |
| B2   | `Framework/Network/Handlers/GameStatusHandler.cs` | 22   | 入门 | 最简单的 Handler 实现，一看就懂。看完再扫一眼其他 Handler，会发现它们都是同一个模板                                       |
| B3   | `Framework/Network/ProtobufManager.cs`            | 96   | 进阶 | 数据仓库 + 观察者模式。重点看 `UpdateData<T>` 的反射赋值和 `OnDataUpdated` 事件                                           |
| B4   | `Framework/Network/MessageDispatcher.cs`          | 121  | 进阶 | Topic → Handler 的路由器。学习零拷贝 `ArraySegment<byte>` 和日志节流                                                      |
| B5   | `Framework/Network/MqttClientService.cs`          | 196  | 进阶 | MQTT 客户端封装。重点看**自动重连协程**和**异步发送队列**                                                                 |
| B6   | `Framework/Network/UdpClientService.cs`           | 278  | 高级 | **必读。** UDP 接收的线程安全设计：后台线程阻塞接收 → ConcurrentQueue → 主线程协程分发。包含 IPv6 双栈和 ArrayPool 对象池 |
| B7   | `Framework/Network/NetworkManager.cs`             | 307  | 高级 | 串联以上所有模块的核心。重点看 `RegisterAndSync<THandler, TMsg>()` 泛型注册方法                                           |

**学习收获**：接口抽象与策略模式；线程安全的生产者-消费者模型；泛型 + 反射的工厂方法；Unity 协程在网络编程中的应用。

**练习建议**：在 B2 之后，尝试不看源码，自己写一个新 Handler（比如 `PenaltyInfoHandler`），然后与实际代码对比。

---

### 阶段 C：骨架 —— MVVM 框架（约 2 小时）

> **目标**：理解 ViewModel 和 View 的抽象层如何工作。

| 序号 | 文件                                           | 行数 | 难度 | 要点                                                                                    |
| ---- | ---------------------------------------------- | ---- | ---- | --------------------------------------------------------------------------------------- |
| C1   | `UI/ViewModels/ProtoViewModelBase.cs`          | 42   | 入门 | ViewModel 基类：订阅 ProtobufManager → `UpdateFrom()` 模板方法 → `PropertyChanged` 通知 |
| C2   | `UI/ViewModels/RobotDynamicStatusViewModel.cs` | 54   | 入门 | 手写 ViewModel 范例，13 个字段的一一映射                                                |
| C3   | `UI/ViewModels/ProtoAutoViewModel.cs`          | 136  | 进阶 | 自动 ViewModel：反射提取 Protobuf 字段，零代码生成属性列表                              |
| C4   | `UI/ViewModels/EventViewModel.cs`              | 7    | 入门 | **仅 7 行**——继承 ProtoAutoViewModel 即完成，极致简洁                                   |
| C5   | `UI/Views/ProtoViewBase.cs`                    | 60   | 进阶 | View 基类：生命周期管理 + 脏标记刷新 + 反射绑定                                         |
| C6   | `UI/Views/KeyboardMouseControlView.cs`         | 35   | 入门 | 手写 View 范例，7 个 TMP_Text 绑定                                                      |
| C7   | `UI/Views/ProtoAutoView.cs`                    | 71   | 进阶 | 自动 View：遍历 ViewModel 属性列表拼接字符串                                            |
| C8   | `UI/ViewModels/GameStatusViewModel.cs`         | 120  | 入门 | 早期手写 ViewModel（未用基类），对比 C2 体会架构演进                                    |

**学习收获**：MVVM 模式在 Unity 中的实现方式；泛型基类 + 模板方法的框架设计；C# 反射在运行时自动发现属性；`INotifyPropertyChanged` 事件驱动 UI 刷新。

**练习建议**：对比 C2（手写）和 C4（自动），思考两者的取舍；尝试用 ProtoAutoViewModel 为一个新消息类型创建调试视图。

---

### 阶段 D：皮肤 —— UI 基础设施（约 2 小时）

> **目标**：理解纯代码 UI 的构建方式。本项目**零预制体**，所有 UI 在运行时通过代码创建。

| 序号 | 文件                         | 行数 | 难度 | 要点                                                                                              |
| ---- | ---------------------------- | ---- | ---- | ------------------------------------------------------------------------------------------------- |
| D1   | `UI/Core/UIColors.cs`        | 42   | 入门 | 全局颜色常量，速览即可                                                                            |
| D2   | `UI/Core/UIFactory.cs`       | 292  | 进阶 | **核心工厂**——CreateCanvas / CreateText / CreateButton 等。重点看 SDF 字体预加载和动态 Atlas 预热 |
| D3   | `UI/Core/IconManager.cs`     | 74   | 入门 | 图标缓存 + 运行时着色                                                                             |
| D4   | `UI/Core/UIShapeHelper.cs`   | 171  | 进阶 | 程序化生成圆角矩形 / 环形 Sprite（SDF 算法），含 Flyweight 缓存                                   |
| D5   | `UI/Core/UISkew.cs`          | 51   | 入门 | BaseMeshEffect 顶点修改，制造平行四边形效果                                                       |
| D6   | `UI/Core/UILayoutManager.cs` | 232  | 进阶 | HUD 布局参数持久化（JSON），定义 HUDSettings 数据结构                                             |
| D7   | `UI/UIManager.cs`            | 153  | 入门 | 弹窗管理器：注册/注销/层级排序                                                                    |

**学习收获**：纯代码构建 Unity UI 的完整工程实践；SDF 在 UI 形状生成中的应用；TextMeshPro 字体预加载 / Atlas 预热优化；Flyweight 模式避免重复创建资源。

---

### 阶段 E：血肉 —— 战斗 HUD（约 4 小时）

> **目标**：阅读最核心的业务代码，理解各个 HUD 组件如何协作。
>
> **建议顺序**：先读简单独立组件，再读编排中心，最后攻克复杂组件。

| 序号 | 文件                                 | 行数 | 难度 | 要点                                                                                |
| ---- | ------------------------------------ | ---- | ---- | ----------------------------------------------------------------------------------- |
| E1   | `UI/HUD/HUDBoot.cs`                  | 61   | 入门 | HUD 启动器：等待兵种选择 → 创建 BattleHUD                                           |
| E2   | `UI/HUD/HealthBarHUD.cs`             | 150  | 入门 | 血条：颜色渐变（绿→黄→红）+ 平滑动画                                                |
| E3   | `UI/HUD/NotificationHUD.cs`          | 185  | 入门 | 通知队列：FIFO + 淡出动画                                                           |
| E4   | `UI/HUD/DamageVignetteHUD.cs`        | 151  | 进阶 | **推荐精读**——运行时生成径向渐变纹理，smoothstep 抗锯齿。学习程序化纹理生成         |
| E5   | `UI/HUD/CrosshairRingHUD.cs`         | 408  | 进阶 | 准星环：热量内环 + 弹药外环 + 数值面板。学习 Radial 360 填充                        |
| E6   | `UI/HUD/AimZoomHUD.cs`               | 231  | 进阶 | 射击开镜：通过修改 RawImage.uvRect 实现数字变焦，无需相机                           |
| E7   | `UI/HUD/DeathOverlayHUD.cs`          | 368  | 进阶 | 死亡画面：3 态状态机 (Alive → Dead → Reviving) + 金币买活                           |
| E8   | `UI/HUD/HexPopupHUD.cs`              | 309  | 进阶 | 六边形弹出提示：程序化六边形 SDF + 3 段动画                                         |
| E9   | `UI/HUD/BattleHUD.cs`                | 459  | 高级 | **HUD 主控中心**——组合所有子组件，每帧从 ViewModel 拉取数据。**读完前面再来读这个** |
| E10  | `UI/HUD/MatchInfoHUD.cs`             | 536  | 高级 | 对局信息栏：动态布局算法（根据可见段计算宽度 + 自动排列 + 分割线）                  |
| E11  | `UI/HUD/BuffStatusHUD.cs`            | 651  | 高级 | BUFF 状态栏：20 种 BUFF 定义 + 双列排序 + 全屏展开面板                              |
| E12  | `UI/HUD/LobShotHUD.cs`               | 661  | 高级 | **最具挑战性**——自定义二值化视频协议解码 + I/D/Trail 帧 + 像素级渲染管线            |
| E13  | `UI/HUD/EventNotificationService.cs` | 1020 | 高级 | 事件通知中枢：线程安全队列 + 50+ 事件类型的 switch 分发                             |
| E14  | `UI/HUD/SettingsPanel.cs`            | 1675 | 高级 | **最大文件**——多级侧边栏 + 10 个参数页面 + 实时预览。可跳读，按需查看感兴趣的页面   |

**学习收获**：状态机在 UI 动画中的应用；程序化纹理 / 形状生成（SDF、smoothstep）；动态布局算法的设计；复杂 UI 编排的分层组合策略；线程安全的事件队列。

---

### 阶段 F：赛前流程 —— 兵种选择（约 1.5 小时）

> **目标**：理解完整的用户交互流程如何编排。

| 序号 | 文件                                             | 行数 | 难度 | 要点                                                        |
| ---- | ------------------------------------------------ | ---- | ---- | ----------------------------------------------------------- |
| F1   | `UI/RobotSelection/RobotSelectionData.cs`        | 88   | 入门 | 数据定义：TeamColor / RobotType 枚举 + RobotSelectionResult |
| F2   | `UI/RobotSelection/RobotCapabilities.cs`         | 196  | 入门 | 兵种能力查表：是否可射击、支持哪种体系选择                  |
| F3   | `UI/RobotSelection/RobotSelectionViewModel.cs`   | 167  | 入门 | 标准 MVVM ViewModel：实现 INotifyPropertyChanged            |
| F4   | `UI/RobotSelection/RobotSelectionPanel.cs`       | 377  | 进阶 | 兵种选择面板：9 个倾斜按钮 + 键盘快捷键 + 数据绑定          |
| F5   | `UI/RobotSelection/PerformanceSelectionPanel.cs` | 523  | 进阶 | 体系选择面板：根据兵种动态构建选项                          |
| F6   | `UI/RobotSelection/RobotSelectionBootstrap.cs`   | 192  | 进阶 | **流程编排器**：兵种确认 → 体系选择 → 写入配置 → 启动 HUD   |

**学习收获**：完整的 MVVM 数据绑定实践（View 与 ViewModel 之间的交互）；启动流程的编排设计（Bootstrap 模式）；动态 UI 构建（根据数据决定 UI 结构）。

---

### 阶段 G：硬核 —— 视频管线（约 4 小时）

> **目标**：理解实时视频流从 UDP 包到屏幕像素的全链路。这是项目中技术含量最高的部分。

| 序号 | 文件                                            | 行数 | 难度 | 要点                                                                                                        |
| ---- | ----------------------------------------------- | ---- | ---- | ----------------------------------------------------------------------------------------------------------- |
| G1   | `Framework/Video/IVideoTransport.cs`            | 17   | 入门 | 传输层接口，3 个成员                                                                                        |
| G2   | `Framework/Video/IVideoDecoder.cs`              | 36   | 入门 | 解码层接口 + DecodedFrame 数据结构（含 ArrayPool 归还）                                                     |
| G3   | `Framework/Network/Handlers/UdpVideoHandler.cs` | 102  | 高级 | UDP 视频包解析：8 字节包头（小端手动解析）+ ArrayPool 零拷贝                                                |
| G4   | `Framework/Video/UdpAnnexBTransport.cs`         | 59   | 入门 | UDP 传输适配器：订阅 Handler → 推入 Assembler → 触发事件                                                    |
| G5   | `Framework/Video/AnnexBFrameAssembler.cs`       | 266  | 高级 | **推荐精读**——分片组帧算法：帧号回绕处理、丢包超时、对象池、动态超时计算                                    |
| G6   | `Framework/Video/RtspTransport.cs`              | 205  | 进阶 | RTSP 传输备选：ffmpeg 拉流 + AUD NAL 分帧                                                                   |
| G7   | `Framework/Video/FfmpegPipeDecoder.cs`          | 796  | 高级 | **全项目第二复杂文件**——ffmpeg 子进程管道 IPC、硬件加速自动选择与回退、codec 自动检测、看门狗监控、进程重启 |
| G8   | `Framework/Video/VideoStreamService.cs`         | 611  | 高级 | **视频管线调度中心**——传输/解码后端切换、IDR 门控、追帧丢帧、双缓冲、增量 GC、NVDEC 回退                    |
| G9   | `Framework/Boot/HardwareCapabilityDetector.cs`  | 529  | 进阶 | 硬件探测：GPU/CPU/VRAM 识别 → 能力分级 → 推荐加速模式                                                       |
| G10  | `Plugins/NativeVideoBridge.cs`                  | 176  | 进阶 | C++ NVDEC 插件的 C# P/Invoke 桥接层                                                                         |

**学习收获**：视频编码基础（H.265 / AnnexB / NAL / IDR）；网络分片重组算法；进程间通信（stdin/stdout 管道）；硬件加速解码的跨平台抽象；性能优化（零拷贝、对象池、增量 GC）。

---

### 阶段 H：工具箱 —— 基础设施（约 1.5 小时）

> **目标**：理解项目的日志、配置、调试工具体系。

| 序号 | 文件                             | 行数 | 难度 | 要点                                                                                                |
| ---- | -------------------------------- | ---- | ---- | --------------------------------------------------------------------------------------------------- |
| H1   | `Utils/Log.cs`                   | 615  | 高级 | **统一日志框架**——后台线程异步写入、编译期宏裁剪、节流去重、10MB 日志轮转、ConcurrentQueue 无锁队列 |
| H2   | `Utils/LogBridge.cs`             | 279  | 进阶 | 编译期分类宏桥接：分类 x 级别的双重裁剪                                                             |
| H3   | `Utils/DebugLog.cs`              | 107  | 入门 | 旧日志 API 适配层（Adapter 模式）                                                                   |
| H4   | `Utils/DebugTools.cs`            | 132  | 入门 | 另一个旧日志兼容层                                                                                  |
| H5   | `Utils/ConfigLoader.cs`          | 257  | 进阶 | 配置加载：按硬件等级选档 + JSON 注释移除 + BOM 处理                                                 |
| H6   | `Editor/ParamsManagerWindow.cs`  | 195  | 进阶 | 编辑器参数管理窗口：可视化编辑 params.json                                                          |
| H7   | `Editor/LogViewerWindows.cs`     | 195  | 进阶 | 编辑器日志查看器                                                                                    |
| H8   | `Framework/Boot/RuntimeTuner.cs` | 86   | 入门 | 运行时调优：VSync、帧率、FPS 诊断                                                                   |
| H9   | `Framework/Boot/StartupUpdateChecker.cs` | 1122 | 高级 | **热更新框架**——全驱 manifest diff、断点续传、SHA-256 校验、平台交换脚本。主路径：`CheckRoutine` → `ComputeDiff` → `DownloadRoutine` → `LaunchSwapAndRestart`。配套阅读《客户端热更新框架》 |

**学习收获**：高性能日志系统的设计（异步 I/O、节流、轮转）；条件编译宏的工程实践；Unity Editor 自定义窗口开发；配置文件的多环境策略；进程外脚本原子替换的跨平台实现。
...
| 序号 | 文件                                   | 行数 | 难度 | 要点                                       |
| ---- | -------------------------------------- | ---- | ---- | ------------------------------------------ |
| I1   | `Services/SimulationInputService.cs`   | 99   | 入门 | 编辑器仿真射击，条件编译 `UNITY_EDITOR`    |
| I2   | `Services/AmmoPurchaseInputService.cs` | 144  | 入门 | 弹药快捷购买：按兵种区分弹种/数量          |
| I3   | `Services/AutoResupplyService.cs`      | 125  | 入门 | 自动补给：监控弹药 → 条件触发 → 批量购买   |
| I4   | `UI/HUD/LobShotService.cs`             | 204  | 进阶 | 吊射模式服务：双 Shift 手势检测 + 模式切换 |
...
### 4.1 Framework/Boot — 启动与硬件检测（3 个文件，1737 行）

| 文件                            | 行数 | 作用                                                                                                                                                           |
| ------------------------------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `StartupUpdateChecker.cs`       | 1122 | MonoBehaviour（ExecutionOrder = -3500，RuntimeInitializeOnLoadMethod 自动注入）。热更新框架：拉取远端 manifest、比对差异、Unity 内下载并 SHA-256 校验、暂存后通过平台脚本原子替换并重启。含 SettingsPanel 风格提示弹窗和图片轮播进度页。详见《客户端热更新框架》文档 |
...
1. **先读接口，后读实现**。例如先看 `IVideoDecoder`（36 行），再看 `FfmpegPipeDecoder`（796 行），就不会迷失在细节中。

2. **先读小文件，后读大文件**。从 `EventViewModel`（7 行）开始理解 ViewModel 模式，比直接看 `GameStatusViewModel`（120 行）更高效。

3. **跟着数据流走**。选一条数据链路（如"比赛状态"），从 `GameStatusHandler` → `ProtobufManager` → `GameStatusViewModel` → `GameStatusView` 一路追踪，理解完整的端到端流程。

4. **善用 IDE 的"查找引用"**。看到一个事件定义后，Ctrl+Shift+F 搜索谁在订阅它。

### 6.2 练习项目

在阅读完各阶段后，可以尝试以下练习（难度递增）：

| 练习                                              | 预计时间 | 涉及阶段 |
| ------------------------------------------------- | -------- | -------- |
| 新增一个 Protobuf Handler + 自动 ViewModel + View | 30 分钟  | B + C    |
| 手写一个 ViewModel，自定义格式化显示              | 1 小时   | C        |
| 实现一个新的 HUD 子组件（如"队友状态面板"）       | 2 小时   | D + E    |
| 为 SettingsPanel 添加一个新的设置页               | 1 小时   | E        |
| 实现一个新的 IVideoTransport（如 WebSocket 传输） | 3 小时   | G        |

### 6.3 需要的前置知识

| 知识领域   | 最低要求                                | 推荐补充                           |
| ---------- | --------------------------------------- | ---------------------------------- |
| C# 语言    | 类、接口、泛型、事件、Lambda            | 反射、async/await、ConcurrentQueue |
| Unity 引擎 | MonoBehaviour 生命周期、协程、Canvas UI | Texture2D、RawImage、RenderTexture |
| 网络编程   | TCP/UDP 概念                            | MQTT 协议、Socket 编程、线程安全   |
| 视频编码   | （可在阶段 G 中边读边学）               | H.264/H.265、AnnexB 格式、NAL Unit |
| 设计模式   | 单例、观察者                            | 策略、工厂、中介者、MVVM           |