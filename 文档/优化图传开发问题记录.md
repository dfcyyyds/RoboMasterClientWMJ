...
12. [Intel 集显环境下的图传适配](#12-intel-集显环境下的图传适配)
...
flowchart TB
    subgraph Main["主线程"]
        A["nvdec_init()"]
        A --> B["cuCtxCreate()\n创建 CUDA 上下文"]
        B --> C["cuvidCtxLockCreate()\n创建 CUVID 上下文锁"]
        C --> D["cuvidCreateDecoder()\n解码器使用上下文锁"]
        D --> E["cuvidCreateVideoParser()"]
    end

    subgraph Parser["解析器内部线程（由 CUVID 创建）"]
        P["cuvidParseVideoData()"]
        P --> Q["HandlePictureDisplay()"]
        Q --> L["cuvidCtxLock()\n✅ 获取上下文锁"]
        L --> M["cuvidMapVideoFrame()\n映射解码帧（此处应可工作）"]
        M --> N["cuvidUnmapVideoFrame()"]
        N --> U["cuvidCtxUnlock()\n释放上下文锁"]
    end
```
...
| Intel 驱动  | iHD driver 24.1.0 (VAAPI)                        |
...
6. **Intel 集显环境必须禁用 NVDEC**：HardwareCapabilityDetector 已自动处理，但手动测试时应确认 `prime-select query` 输出与预期一致。

7. **VAAPI 驱动版本**：Intel 集显环境下应安装 iHD 驱动 23.0 以上版本以获得完整的 HEVC 解码支持。

8. **GPU 切换后重启**：`prime-select intel/nvidia` 切换后必须重启或至少注销重新登录，否则图形栈状态不一致。
...
```mermaid
sequenceDiagram
    participant Main as 主线程（Unity）
    participant CUVID as CUVID 线程（回调）

    Main->>Main: nvdec_init()
    Note over Main,CUVID: cuPbo 在主线程延迟注册

    CUVID-->>CUVID: 首帧数据到达
    CUVID->>CUVID: HandlePictureDisplay()
    alt cuPbo 尚未注册
        CUVID->>CUVID: ctx->cuPbo == nullptr → 跳过
    end

    Main->>Main: nvdec_get_texture()
    Main->>Main: cuGraphicsGLRegisterBuffer() 注册 cuPbo

    CUVID-->>CUVID: 后续帧数据到达
    CUVID->>CUVID: HandlePictureDisplay()
    alt cuPbo 已注册
        CUVID->>CUVID: ctx->cuPbo != nullptr → 正常处理
    end
```
...
项目需要支持仅有 Intel 集显的低配环境部署。在 Intel 集显模式下（`prime-select intel` 或无独显设备），CUDA 和 NVDEC 完全不可用，原生 NVDEC 插件无法加载，需要确保图传系统自动降级到可用的解码路径。

### 症状

- `NativeVideoBridge.Init()` 抛出 `DllNotFoundException` 或 `EntryPointNotFoundException`
- ffmpeg 报错：`cu->cuInit(0) failed` — CUDA 设备不存在
- `nvidia-smi` 无输出或报错 `NVIDIA-SMI has failed`
- Unity 控制台显示：`[WARN] Native NVDEC init 失败，回退 Ffmpeg`
...
Intel 集显不支持 CUDA/NVDEC API，这是硬件限制而非软件错误。系统的图传方案需要三级降级策略：

```mermaid
flowchart TD
    A[VideoStreamService 启动] --> B{HardwareCapabilityDetector}
    B -->|检测到 NVIDIA GPU| C[NativeNvdec 后端]
    B -->|无 NVIDIA GPU| D[FfmpegPipe 后端]
    C -->|初始化失败| D
    D --> E{ffmpeg 硬件加速检测}
    E -->|检测到 VAAPI| F[ffmpeg + VAAPI 硬解]
    E -->|检测到 CUDA| G[ffmpeg + CUDA 硬解]
    E -->|无硬件加速| H[ffmpeg 纯软解]
    F --> I[正常解码]
    G --> I
    H --> I
```
...
`HardwareCapabilityDetector` 在 `VideoStreamService.Awake()` 中调用，检测当前环境推荐的加速方式：

```csharp
var recommended = HardwareCapabilityDetector.RecommendedAcceleration;
if (recommended != HwAccelType.Nvdec)
{
    decodeBackend = DecodeBackend.FfmpegPipe;  // 自动降级
}
```

#### 2. FfmpegPipeDecoder VAAPI 路径（已实现）

当检测到 VAAPI 可用时，ffmpeg 使用以下参数启动：

```
-hwaccel vaapi -vaapi_device /dev/dri/renderD128 -extra_hw_frames 8
```

解码后经 `scale_vaapi` 滤镜缩放，再 `hwdownload` 到 CPU 内存。

#### 3. NativeVideoBridge 安全机制（已实现）
...
{
    try { return nvp_init(w, h); }
    catch (DllNotFoundException) { Available = false; return -1; }
    catch (EntryPointNotFoundException) { Available = false; return -1; }
}
```

### 测试数据

在 Intel Arrow Lake-U 集显环境下进行了 86 秒稳定性测试（详见 [Intel集显低配测试报告.md](Intel集显低配测试报告.md)）：
...
| Exception 异常       | 0 条                         |
| GPU 渲染利用率       | 50.79%（平均）               |
| VAAPI 视频引擎利用率 | 12.25%（平均），100%（峰值） |
| CPU 使用率           | 13.79%（平均）               |
| 内存使用             | 13GB / 30GB                  |

### Intel VAAPI 支持的编解码格式

| 编解码器        | 解码 | 编码 |
| --------------- | ---- | ---- |
| H.264 Main/High | 支持 | 支持 |
| HEVC Main       | 支持 | 支持 |
| VP9             | 支持 | 支持 |
| AV1             | 支持 | 支持 |
...
| 指标       | NVIDIA RTX 5080 (NVDEC) | NVIDIA RTX 5080 (ffmpeg+CUDA) | Intel ARL (ffmpeg+VAAPI)   |
| ---------- | ----------------------- | ----------------------------- | -------------------------- |
| 解码路径   | NativeNvdec 零拷贝      | FfmpegPipe + CUDA hwaccel     | FfmpegPipe + VAAPI hwaccel |
| GPU 利用率 | 低                      | 中                            | 50.79%                     |
| CPU 占用   | 极低                    | 低                            | 13.79%                     |
| 解码延迟   | 20-40ms                 | 50-80ms                       | 待测                       |
| 纹理上传   | 5-10ms (GPU 内零拷贝)   | 20-30ms (需下载到 CPU)        | 20-30ms (需下载到 CPU)     |

### 已知限制

1. **VAAPI 路径仍需 CPU 拷贝**：ffmpeg VAAPI 解码后需要 `hwdownload` 到 CPU 内存再上传到 Unity 纹理，无法实现 GPU 内零拷贝
2. **无 Intel 原生零拷贝方案**：未实现类似 NVDEC 的 Intel VAAPI-GL 互操作路径（技术上可行但优先级低）
3. **GPU 功耗较高**：Intel 集显在持续解码时平均功耗 67W，峰值 91W

### 相关文件

- `Assets/Scripts/Framework/Video/VideoStreamService.cs` — 后端选择与降级逻辑
- `Assets/Scripts/Framework/Video/FfmpegPipeDecoder.cs` — VAAPI 参数生成与硬解回退
- `Assets/Scripts/Framework/Video/HardwareCapabilityDetector.cs` — GPU 能力检测
- `Assets/Scripts/Plugins/NativeVideoBridge.cs` — DllNotFound 安全处理
